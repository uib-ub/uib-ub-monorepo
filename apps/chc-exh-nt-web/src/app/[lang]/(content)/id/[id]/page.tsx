...
import { Description } from '@/src/app/components/Props/Description'
import { CodeBracketSquareIcon, IdentificationIcon, SwatchIcon } from '@heroicons/react/24/outline'
import ManifestViewer from "@/src/app/components/IIIF/ManifestViewer"
import { Subjects } from '@/src/app/components/Props/Subjects'
import { Palette } from '@/src/app/components/Props/Palette'
...
export async function generateMetadata({ params: { lang, id } }: PageProps) {
...
export default async function ItemPage({ params: { lang, id } }: PageProps) {
  const item = await getItem(id, lang)
    ...
    <div className='flex flex-col md:flex-grow min-h-screen p-5'>
      ...
      {item.manifest && (
        <div className='min-h-[65vh] mb-5 border border-neutral-300 dark:border-neutral-700 shadow-lg bg-neutral-100 dark:bg-neutral-900'>
          <ManifestViewer
            iiifContent={item.manifest.replace('api-ub.vercel.app', 'api.ub.uib.no').replace('/manifest', '?as=iiif')}
            options={{
              canvasHeight: '65vh',
              showIIIFBadge: false,
              showTitle: false,
