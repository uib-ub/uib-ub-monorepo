...
const getSubtitle = (slug: string, link: string, route: string) => {
  if (slug) {
    return ['/', slug].join('')
  }
  if (link) {
...
function useAsyncSlugifier(input: any) {
...
export default defineType({
  ...
  preview: {
    ...
    prepare({ slug, label, pageLabel, link, route }: { slug: string, label: string, pageLabel: string, link: string, route: string }) {
      return {
        title: coalesceLabel(label) || pageLabel,
        subtitle: getSubtitle(slug, link, route) ?? undefined,
      }
    },
  },
})
