...
export default function Menu() {
    ...
    return (
        <div ref={menuRef} className="bg-neutral-50 z-[6000] flex gap-1 items-center justify-center h-full border-l-2 border-neutral-200 xl:border-none">
            <button aria-controls="menu_navbar" 
                        onBlur={handleBlur}
                        aria-label="Meny"
                        aria-expanded={menuOpen} 
                        className="items-center justify-center flex h-full w-14 xl:w-12 bg-neutral-50" 
                        onClick={() => setMenuOpen(!menuOpen)}>
                <PiList className="text-3xl"/></button>
 
                <div 
                     id="menu_navbar" 
                     className={` bg-neutral-50 overscroll-none h-[calc(100svh-3rem)] border-t border-neutral-200 overflow-y-auto ${menuOpen ? 'fixed top-14 bottom-0 left-0' : 'hidden'}`}>
                ...
                <NavBar 
                        onBlur={handleBlur}
                        className="text-3xl text-center py-6 uppercase flex flex-col border-t border-neutral-200 tracking-wider "/>
                </div>
