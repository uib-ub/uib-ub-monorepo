...
export default function NavBar({ handleBlur, ...props }: any) {
    ...
    return (
        <nav id="top " {...props}>
            ...
            <Link onBlur={handleBlur} scroll={false} className="py-3 mx-3 lg:py-1 lg:px-2 lg:mx-0" href={`/search${pinnedFilters[dataset]?.length > 0 ? `?${new URLSearchParams(pinnedFilters[dataset]).toString()}` : ''}`}>Overordna søk</Link>
