...
export async function GET(request: Request) {
    ...
    if (data._source.type === 'Collection') {
        ...
        if (fmt === 'jpgs') {
            ...
            for (const hit of hits) {
                ...
                for (let i = 0; i < images.length; i++) {
                    const img = images[i];
                    const imageUrl = `https://iiif.test.ubbe.no/iiif/image/stadnamn/${itemDataset.toUpperCase()}/${img.uuid}/full/max/0/default.jpg`;
        ...
        if (fmt === 'multipdf') {
            ...
            for (const hit of hits) {
                ...
                const addImageToPdf = async (canvas: any, pageIndex: number, pdf: jsPDF) => {
                    const imageUrl = `https://iiif.test.ubbe.no/iiif/image/stadnamn/${itemDataset.toUpperCase()}/${canvas.uuid}/full/max/0/default.jpg`;
        ...
        const addImageToPdf = async (
            ...
            const imageUrl = `https://iiif.test.ubbe.no/iiif/image/stadnamn/${canvas.dataset.toUpperCase()}/${canvas.uuid}/full/max/0/default.jpg`;
            const response = await fetch(imageUrl);
            if (!response.ok) {
