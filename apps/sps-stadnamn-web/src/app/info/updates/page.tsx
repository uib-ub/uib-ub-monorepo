...
export default async function Updates({searchParams}: {searchParams: Promise<{dataset: string}>}) {
  ...
  const updates = await fetch(`https://git.app.uib.no/api/v4/projects/26634/repository/commits?ref_name=main${dataset ? `&path=lfs-data/elastic/${dataset}_elastic.json` : ''}`, {cache: 'no-store'})
