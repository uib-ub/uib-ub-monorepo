  ...
  return (
    ...
    <ul className='flex flex-col gap-1 overflow-auto md:mx-1'>
      {resultData.hits.map(hit => (
        <li key={hit._id} className="my-0 rounded-sm p-2 flex-grow "><Link className="no-underline font-semibold" href={documentUrl(hit._id)}>{hit._source.label}</Link> | {hit._source.rawData.kommuneNamn}
        </li>
      ))}
    </ul>
