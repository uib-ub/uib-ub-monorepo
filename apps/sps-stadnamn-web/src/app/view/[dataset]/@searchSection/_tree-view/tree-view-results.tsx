                ...
                Array.from({length: isLoading && size == 50 ? 20 : (hits?.total && hits.total.value < size ? hits.total.value : size + 1)}, (_, i) => {
                    ...
                    if (hit) {
                        ...
                        return <li key={i} className="flex gap-4 px-2 py-2 border-b border-neutral-300 mx-2">
                            <div className="rounded-md mt-1 mb-3  bg-neutral-200 h-[1em] animate-pulse" style={{width: `${Math.floor(Math.random() * (24 - 6 + 1) + 6)}rem`}}></div><div className="h-[1.5em] w-[1.5em] rounded-full self-center bg-neutral-200 ml-auto animate-pulse"></div>
                        </li>
                    }
                    else if ( i == size) {
                        return <li key={i} className="flex gap-4 px-2 py-8 mx-2 justify-center"><Link className="flex self-center gap-4 px-2 py-2 border rounded-sm border-neutral-300 mx-2 no-underline py-4 px-16 text-xl self-center" href={ nextPageUrl() }>Vis mer <PiCaretDown className="self-center" aria-hidden="true"/></Link></li>
