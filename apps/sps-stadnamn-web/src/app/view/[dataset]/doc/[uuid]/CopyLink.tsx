...
export default function CopyLink({ uuid }: { uuid: string }) {
  ...
  const copyLink = async () => {
    ...
    return (
        ...
        { params.dataset && <Link href={process.env.NODE_ENV == 'development' ?  `/uuid/${uuid}` : `https://purl.org/stadnamn/uuid/${uuid}`} className="no-underline inline flex gap-1 items-center whitespace-nowrap"><PiInfinityBold aria-hidden="true" className="inline"/>Varig infoside</Link>}
