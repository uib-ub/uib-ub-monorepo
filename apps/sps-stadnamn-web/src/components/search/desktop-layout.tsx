...
import { PiArchive, PiArrowUpLeft, PiCaretDownBold, PiCaretUpBold, PiFiles, PiFileSqlThin, PiFilesThin, PiFolder, PiTable, PiTag, PiXBold } from "react-icons/pi"
        ...
        { docData?._source?.children?.length > 1 || parentData?._source?.children?.length > 1 || (treeSettings[dataset] && docData?._source?.sosi == 'gard') || parent ?
         <div className="rounded-md shadow-md !z-[3001] bg-white  flex flex-col instance-info justify-start">
            ...
            <h2 className=""><ParamLink className="flex gap-2 no-underline p-2 px-4 justify-start items-center" add={{parent: docData?._source.uuid}}>
             ...
             <><PiFiles className="text-neutral-800 text-2xl"/><span className="text-xl">Kilder</span><div className="!h-6 self-center text-base font-bold bg-neutral-200 text-neutral-950 rounded-full px-2 !font-sans">{childrenData?.length}</div></>
            ...
            <div className="flex">
                <h2 className="flex gap-2 no-underline justify-start items-center">
                    <ParamLink className="flex gap-2 no-underline p-2 px-4 justify-start items-center" add={{parent: docData?._source.uuid}}>
                    <PiTable className="text-neutral-800 text-2xl"/><span className="text-xl">Matrikkeltabell</span>
                    </ParamLink>
                    {childrenLoading && <Spinner status="Laster matrikkeltabell"/>}
                </h2>
                </div>
