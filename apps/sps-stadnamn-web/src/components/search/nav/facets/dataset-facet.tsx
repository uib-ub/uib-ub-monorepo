...
export default function DatasetFacet() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/facet?perspective=all&facets=dataset${facetSearch ? '&facetSearch=' + facetSearch + "*" : ''}${paramsExceptFacet ? '&' + paramsExceptFacet : ''}${sortMode != 'doc_count' ? '&facetSort=' + sortMode : ''}`).then(response => response.json()).then(es_data => {
