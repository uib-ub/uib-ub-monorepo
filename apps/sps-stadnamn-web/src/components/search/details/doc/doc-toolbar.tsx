...
export default function DocToolbar({ docData }: { docData: any }) {
  ...
  return <div className="flex h-10 gap-2">
    <IconLink label="Opne infoside" href={`${process.env.NODE_ENV == 'development' ? '' : 'https://stadnamn.no'}/uuid/${docData?._source?.uuid}`} className="btn btn-outline btn-compact flex items-center gap-2">
