...
export function getLabelMarkerIcon(
  ...
  showPin?: boolean,
  options?: {
    /** Render as small outlined text (no background bubble). */
    smallText?: boolean
  },
) {
  const colorValue = colorMapping[color] || color
  const smallText = Boolean(options?.smallText)

  // Thin white outline without a background box.
  // `text-shadow` is widely supported and works over imagery.
  const outlinedTextStyle =
    'color:#000; font-weight:600; text-shadow:' +
    '0.75px 0 #fff, -0.75px 0 #fff, 0 0.75px #fff, 0 -0.75px #fff,' +
    '0.75px 0.75px #fff, 0.75px -0.75px #fff, -0.75px 0.75px #fff, -0.75px -0.75px #fff;'
  return {
    className: '',
    html: `
        <div class="map-marker group" role="button" tabindex="0" style="display: flex; align-items: center; justify-content: center; position: relative; height: 2rem;">
          <div class="absolute -top-6 left-1/2 -translate-x-1/2">
            <div class="flex flex-col items-center">
              ${
                smallText
                  ? `<div class="flex items-center tracking-wide whitespace-nowrap text-center text-sm" style="${outlinedTextStyle}">
                       <div class="${!showPin ? 'max-w-48 min-w-0 truncate' : ''} block">${label}</div>
                       ${docCount ? `<span class="ml-1 text-xs flex items-center py-0 my-0.5 font-semibold" style="${outlinedTextStyle}">+${docCount}</span>` : ''}
                     </div>`
