        ...
        {filteredItems.map((item: Record<string,any> , index: number) => (
            <div key={index} className="flex flex-col">
                ...
                <p>{item.searchFields && Object.values(item.searchFields).every(value => typeof value === 'string') ?
                  ...
                  <Link className="no-underline flex items-center gap-1" href={`/view/${dataset}?${Object.entries(item.searchFields).map(([key, value]) => `${key}=${encodeURIComponent(value as string)}`).join('&')}`}>{item.value}<div></div><PiMagnifyingGlass className="inline text-primary-600"/></Link>
                  : item.value}</p>
