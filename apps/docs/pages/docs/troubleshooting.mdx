...
## Vercel looks for missing linux build for some pagckages

For some reason Vercel does not install linux pagckages for turbo, esbuild and @napi-rs/simple-git. Solution is to add them to the package-lock.json file, but there are som black magic here...
