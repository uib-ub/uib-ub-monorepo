<template>
  <header class="bg-primary pl-3 pr-0 lg:px-5 py-1 flex flex-col lg:flex-row content-center text-white shadow-[rgba(0,_0,_0,_0.24)_0px_3px_8px]">
    ...
    <NuxtLink class="navbar-brand" to="/" :aria-current="$route.name == 'dict' && 'page'">
        ...
        <p class="hidden xl:block byrand-subtitle ml-0.5 mb-1">{{$t("sub_title")}}</p>
        </div>
      </div>
        </NuxtLink>
...
/* Text Size Hover */
  /* .hover-text {
  position:absolute;
  @apply p-4 border-2 border-canvas-darken mt-9 mr-4 font-semibold text-black bg-canvas shadow;
