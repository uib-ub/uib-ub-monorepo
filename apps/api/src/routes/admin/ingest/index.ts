...
import { turnOffRefreshInterval, turnOnRefreshInterval } from '../../../services/es/refreshInterval'
import { getItems } from '../../../services/sparql/legacy/getItems'
import { DOMAIN, dataSources } from '../../../libs/constants'
import { flatMapDataForBulkIndexing } from '../../../libs/helpers/flatMapDataForBulkIndexing'
import { resolveIds } from '../../../libs/helpers/resolveIds'
import { indexData } from '../../../libs/helpers/indexData'
import { cleanJsonld } from '../../../libs/helpers/cleanJsonLd'
...
app.get('/ingest', async (c) => {
  ...
  const API_URL = dataSources.filter(service => service.name === source)[0].url
  ...
  return streamSSE(c, async (stream) => {
    ...
    await stream.writeSSE({
      data: 'Turned off refresh interval',
      event: 'prepare',
      id: String(id++),
    })
