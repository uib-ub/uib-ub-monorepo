...
export async function lookupId(id: string) {
  ...
  const sparqlPromises = dataSources.filter(s => s.type === 'sparql').map(async (service: any) => {
  ...
  const goobiPromise = dataSources.filter(s => s.name === 'samla').map(async (service: any) => {
    return fetch(`${service.lookupUrl || service.url}`, { method: 'POST', body: goobiQuery, headers: { "content-type": "application/json" } }).then(res => res.json()).then(res => {
      console.log(res)
      if (res.numFound) {
        return service
      }
