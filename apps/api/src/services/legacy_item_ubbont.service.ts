...
function getQuery(id: string) {
  const query = `
    ...
    CONSTRUCT {
      ...
      FILTER(?p NOT IN (rdf:type, ubbont:cataloguer, ubbont:internalNote, ubbont:showWeb, ubbont:clause, ubbont:hasRepresentation, ubbont:hasThumbnail, dct:relation, dc:relation))
    }`

  return query
}
async function getItemData(id: string, source: string, context: string, type: string): Promise<JsonLd> {
