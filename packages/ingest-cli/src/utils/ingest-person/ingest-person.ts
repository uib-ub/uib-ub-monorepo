...
export const ingestPerson = async (id: string) => {
  ...
  const currentIndex = Object.keys(indicesInAlias).find(k => k.startsWith(CHC_INDICIES.people))

  if (!currentIndex) {
    throw new Error(`No index found for ${CHC_INDICIES.people}`)
  }

  const response = await fetchAndProcessPerson(id)
